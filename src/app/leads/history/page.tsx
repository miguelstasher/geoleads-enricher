...
const UserDisplay = ({ userId }: { userId?: string }) => {
  ...
  if (!user) {
    ...
    if (userId && userId !== '') {
      // Try to get the photo from the current user's profile
      const photoUrl = currentUser?.profile?.photo_url;
      
      return (
        <div className="flex items-center space-x-2">
          <div className="w-6 h-6 rounded-full overflow-hidden bg-blue-500 flex items-center justify-center">
            {photoUrl ? (
              <img 
                src={photoUrl} 
                alt="Miguel Elias"
            ...
            <div className="w-full h-full bg-blue-500 rounded-full flex items-center justify-center text-xs text-white font-medium" style={{display: photoUrl ? 'none' : 'flex'}}>
              ME
            </div>
          </div>
          <span className="text-sm text-gray-900">Miguel Elias</span>
        </div>
      );
    }
