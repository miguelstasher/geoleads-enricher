...
export async function POST(request: NextRequest) {
  try {
    ...
    // Filter out leads that need enrichment
    const leadsToEnrich = leads.filter(lead => {
      // Must have website
      const hasWebsite = lead.website && lead.website.trim() !== '';
      
      // Must not have email or have placeholder/not found email
      const needsEmail = !lead.email || 
                        lead.email.trim() === '' || 
                        lead.email === 'Not Found' || 
                        lead.email === 'not_found' ||
                        lead.email === 'Click to add' ||
                        lead.email.toLowerCase().includes('click to add');
...
async function processEnrichmentJob(jobId: string, leads: any[]) {
  ...
  try {
    ...
    console.log(`📊 Processing ${results.length} enrichment results...`);
    for (let i = 0; i < leads.length; i++) {
      ...
      if (result && result.email && 
        ...
        console.log(`✅ Valid email found: ${result.email} (status: ${result.email_status})`);
        ...
        // No email found after trying all methods - set to "Not Found"
        console.log(`❌ No email found for ${lead.name} after waterfall enrichment`);
        ...
        if (updateError) {
          ...
          console.log(`📝 Updated lead ${lead.name} status: Not Found`);
        }
      }
    }
